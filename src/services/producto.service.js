import { Producto } from "../db.js";
import { buildWhereClause, buildOrderClause } from "../utils/filters.js";

export const createProducto = async (data) => Producto.create(data);
...
export const getProducto = async (id) => {
  const entity = await Producto.findByPk(id);
...
export const deleteProducto = async (id) => {
  const count = await Producto.destroy({ where: { id } });
