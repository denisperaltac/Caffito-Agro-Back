import { Sanidad } from "../db.js";
import { buildWhereClause, buildOrderClause } from "../utils/filters.js";

export const createSanidad = async (data) => Sanidad.create(data);
export const listSanidads = async (options = {}) => {
  ...
  const { count, rows } = await Sanidad.findAndCountAll(queryOptions);

  return { data: rows, total: count };
};
export const getSanidad = async (id) => {
  const entity = await Sanidad.findByPk(id);
...
export const deleteSanidad = async (id) => {
  const count = await Sanidad.destroy({ where: { id } });
