import { Dieta } from "../db.js";
import { buildWhereClause, buildOrderClause } from "../utils/filters.js";

export const createDieta = async (data) => Dieta.create(data);
export const listDietas = async (options = {}) => {
  ...
  const { count, rows } = await Dieta.findAndCountAll(queryOptions);

  return { data: rows, total: count };
};
export const getDieta = async (id) => {
  const entity = await Dieta.findByPk(id);
...
export const deleteDieta = async (id) => {
  const count = await Dieta.destroy({ where: { id } });
